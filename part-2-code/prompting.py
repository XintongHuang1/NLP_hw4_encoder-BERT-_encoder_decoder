...
MAX_NEW_TOKENS = 256  # Maximum number of tokens to generate for SQL queries
...
def create_prompt(sentence, k):
    ...
    '''
    # TODO
    system_prompt = """You are an expert SQL query generator. Convert natural language questions to SQL queries for a flight database.

Database Schema:
- flight(flight_id, airline, source_airport, destination_airport, departure_time, arrival_time)
- airport(airport_code, airport_name, city_code)
- airline(airline_code, airline_name)
- city(city_code, city_name, state)
- fare(flight_id, fare_basis_code, restriction_code, one_direction_cost, round_trip_cost)
...
"""
    ...
    if k == 0:
        ...
        # Few-shot prompting (examples would be added here if provided)
        prompt = f"{system_prompt}\n\nQuestion: {sentence}\nSQL Query:"
...
def eval_outputs(eval_x, eval_y, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    # eval_y should be the list of generated SQL queries (extracted_queries from exp_kshot)
    save_queries_and_records(eval_y, model_sql_path, model_record_path)
    
    # Load ground truth SQL queries if available
    if os.path.exists(gt_sql_pth):
        # Compute metrics using the utility function
        sql_em, record_em, record_f1, model_error_msgs = compute_metrics(
            gt_sql_pth, model_sql_path, gt_record_path, model_record_path
        )
        error_rate = len(model_error_msgs) / len(eval_x) if len(eval_x) > 0 else 0
    else:
...
def main():
    ...
    for eval_split in ["dev", "test"]:
        ...
        # Save logs, if needed
        log_path = f"logs/gemma_{experiment_name}_{eval_split}.log"
