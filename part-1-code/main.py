...
def create_augmented_dataloader(args, dataset):
    ...
    # You may find it helpful to see how the dataloader was created at other place in this code.
        ...
        # Select 5000 random samples from training set for transformation
    samples_to_transform = dataset["train"].shuffle(seed=42).select(range(5000))
    
    # Apply custom transformation to the selected samples
    transformed_samples = samples_to_transform.map(custom_transform, load_from_cache_file=False)
    
    # Concatenate original training set with transformed samples
    from datasets import concatenate_datasets
    augmented_dataset = concatenate_datasets([dataset["train"], transformed_samples])
